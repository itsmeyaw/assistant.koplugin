...
function AnthropicHandler:buildRequestBody(messages, tools, stream)
    ...
    if tools then
        -- Keep configured native tools when a local or external tool is added.
        body.tools = {}
        if type(self.additional_parameters.tools) == "table" then
            for _, tool in ipairs(self.additional_parameters.tools) do
                table.insert(body.tools, tool)
            end
        end
...
function AnthropicHandler:query(message_history, query_option)
    ...
    if query_option.use_stream_mode then
        local stream_tools = ToolExecutor.buildTools("anthropic", ws_mode, query_option.use_booksearch)
    ...
    -- NON-STREAM path
    -- -----------------------------------------------------------------------
    -- Let the Querier handle local and external tool execution.
    local requestBody = self:buildRequestBody(message_history,
        ToolExecutor.buildTools("anthropic", ws_mode, query_option.use_booksearch), false)
